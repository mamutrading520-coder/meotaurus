...
async def do_manage_tasks(content: str, owner: Optional[str] = None) -> Dict:
    ...
    from services.scheduled_tasks.management_service import do_manage_tasks as _tasks_do_manage_tasks

    return await _tasks_do_manage_tasks(content, owner)
...
from services.platform import (
    do_manage_endpoints as _platform_do_manage_endpoints,
    do_manage_mcp as _platform_do_manage_mcp,
    do_manage_webhooks as _platform_do_manage_webhooks,
    do_manage_tokens as _platform_do_manage_tokens,
)

async def do_manage_endpoints(content: str, owner=None):
    return await _platform_do_manage_endpoints(content, owner)

async def do_manage_mcp(content: str, owner=None):
    return await _platform_do_manage_mcp(content, owner)

async def do_manage_webhooks(content: str, owner=None):
    return await _platform_do_manage_webhooks(content, owner)

async def do_manage_tokens(content: str, owner=None):
    return await _platform_do_manage_tokens(content, owner)


async def do_manage_documents(content: str, owner: Optional[str] = None) -> Dict:
    """Manage documents: list, read/view/open, delete, tidy."""
    from services.docs.management_service import do_manage_documents as _docs_do_manage_documents

    return await _docs_do_manage_documents(content, owner)
...
async def do_manage_settings(content: str, owner: Optional[str] = None) -> Dict:
    ...
    from services.settings.management_service import do_manage_settings as _settings_do_manage_settings

    return await _settings_do_manage_settings(
        content,
        owner,
        parse_tool_args=_parse_tool_args,
        logger_obj=logger,
    )
...
async def do_api_call(content: str) -> Dict:
    ...
    from services.api_calls.management_service import do_api_call as _api_calls_do_api_call

    return await _api_calls_do_api_call(content)
...
async def do_manage_notes(content: str, owner: Optional[str] = None) -> Dict:
    ...
    from services.notes.management_service import do_manage_notes as _notes_do_manage_notes

    return await _notes_do_manage_notes(content, owner)
...
async def do_adopt_served_model(content: str, owner: Optional[str] = None) -> Dict:
    ...
    if add_endpoint:
        ...
        try:
            from services.platform import do_manage_endpoints  # avoid forward ref issues
        except Exception:
